<template>
  <div>
    <h1 style="margin: 20px 0; font-size: 24px" >上线资讯</h1>
    ...
    <el-table
      ...
      <el-table-column
        ...
        width="80">
        <template slot-scope="scope">
          <el-button size="mini" type='primary' @click="_downNews(scope.row.id)" >下线</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
import { getMagazinePage, getDocumentList, searchDocument, getTypeList } from '@/api/pastList'
import { downNews } from '@/api/upNews'
...
export default {
  ...
  created () {
    getMagazinePage().then(res => {
      ...
      getTypeList(this.magazine.id).then(res => {
  ...
  methods: {
    ...
    // 下线期刊
    _downNews (id) {
      downNews(id).then(res => {
        this.$message({
          message: res.message,
          type: 'success',
          duration: 3 * 1000
        })
        searchDocument(this.magazine.name, this.typeId).then(res => {
          this.list = res.data
        })
      })
    }
